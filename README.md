...
the contacts API. Fields marked \* are optional on `Contact`; only `company` and `story`
are required.
...
The Docker entrypoint runs `python manage.py fix_data` before migrations to delete older
duplicate rows and keep the newest record before database constraints are applied.
